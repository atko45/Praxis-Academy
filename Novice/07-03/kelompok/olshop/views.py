...
def edit_saldoawal(req):
    if req.POST:
        models.penjualan1m.objects.filter().update(saldo_awal=req.POST['saldo_awal'])
        return redirect('/lr')

    penjualan1 = models.penjualan1m.objects.filter().first()
    return render(req, 'keperluan/index16.html', {
        'penjualan1': penjualan1,
    })
